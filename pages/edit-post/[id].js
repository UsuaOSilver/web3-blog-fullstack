...
export default function Post() {
    ...
    async function fetchPost() {
        /** we first fecth the individual post by ipfs hash from the network */
        if (!id) return 
        let provider
              ...
              editing && (
                <div>
                    <input
                      ...
                      name='title'
                      plceholder='Give it a title ...'
                      value={post.title}
                      className={titleStyle}
                    />
                    <simpleMDE
                      className={mdEditor}
                      placeholder="What's pn your mind?"
                      value={post.content}
...
const button = css`
    ...
    font-size: 18px;
    padding: 16px 70px;
    bow-shadow: 7px 7px regba(0,0,0,0.1);
`
